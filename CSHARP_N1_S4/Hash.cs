        ...
        public void Delete(hashItem _item)
        {
            if (Search(_item.getNumber(), out _item)) //если такой элемент в таблице существует, то применяем первый хеш
            {
                ...
                if (!isEmpty())
                {
                    ...
                    //когда мы выходим из while, элемент точно найден
                    //тк в самом начале Delete есть проверка на его существование в таблице (если бы его не было, в этот цикл не вошли бы)
